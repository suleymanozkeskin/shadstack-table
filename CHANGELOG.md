...
- Corrected a stale claim in the README and migration guide that listed **column drag-reorder** as deferred. The feature has actually shipped since 0.1.0 (`enableColumnOrdering` plus `SST_TableHeadCellGrabHandle`) — only the documentation was wrong. No code change.
