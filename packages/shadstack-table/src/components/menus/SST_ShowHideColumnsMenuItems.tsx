...
import { SST_ColumnPinningButtons } from '../buttons/SST_ColumnPinningButtons';
import { SST_GrabHandleButton } from '../buttons/SST_GrabHandleButton';

export interface SST_ShowHideColumnsMenuItemsProps<
  TData extends SST_RowData,
> extends React.ComponentProps<'div'> {
  allColumns: SST_Column<TData>[];
  column: SST_Column<TData>;
  hoveredColumn: SST_Column<TData> | null;
  isNestedColumns: boolean;
  setHoveredColumn: Dispatch<SetStateAction<SST_Column<TData> | null>>;
  table: SST_TableInstance<TData>;
}

export const SST_ShowHideColumnsMenuItems = <TData extends SST_RowData>({
  allColumns,
  className,
  column,
  hoveredColumn,
    ...
    <>
      ...
      >
        <div className="flex flex-nowrap items-center gap-2">
          ...
          {enableColumnPinning &&
            (column.getCanPin() ? (
              <SST_ColumnPinningButtons column={column} table={table} />
            ) : (
      ...
      {column.columns?.map((c: SST_Column<TData>) => (
