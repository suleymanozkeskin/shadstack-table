...
| `@mui/x-date-pickers` (Date/Time/DateTime)                              | shadcn has no canonical time picker; date picker is a Popover + Calendar recipe.    | `date` / `date-range` use shadcn `Popover` + `Calendar` (react-day-picker v9) via `SST_DateFilter`. `time` / `datetime` / `time-range` / `datetime-range` still fall back to native `<input type=...>`.    |
